...
export namespace ThreeDSRequestorPriorAuthenticationInfo {
	export function is(
		value: ThreeDSRequestorPriorAuthenticationInfo | any
	): value is ThreeDSRequestorPriorAuthenticationInfo {
		return (
			typeof value == "object" &&
			(value.threeDSReqPriorAuthData == undefined ||
				(typeof value.threeDSReqPriorAuthData == "string" && value.threeDSReqPriorAuthData.length <= 2048)) &&
			...
			(value.threeDSReqPriorRef == undefined ||
				(typeof value.threeDSReqPriorRef == "string" && value.threeDSReqPriorRef.length <= 36))
		)
	}
}
