...
export namespace Response {
	export function is(value: Response | any): value is Response {
		return typeof value == "object" &&
			...
			(value.acsInfoInd == undefined || Array.isArray(value.acsInfoInd) && value.acsInfoInd.every((information: string | any) => typeof information == "string" && /^(0[1-4]|[89][0-9])$/.test(information))) &&
			...
			typeof value.endRange == "string" && /^[0-9]{13,19}$/.test(value.endRange) &&
			typeof value.startRange == "string" && /^[0-9]{13,19}$/.test(value.startRange) &&
			(value.threeDSMethodURL == undefined || typeof value.threeDSMethodURL == "string" && value.threeDSMethodURL.length <= 256) &&
